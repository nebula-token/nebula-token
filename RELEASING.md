...
release none of them do, and five of the ten authenticate with OIDC — which is
...
|---|---|
| npm | the `nebula-token` name claimed, and a trusted publisher naming this repository, `release.yml` and the `release-npm` environment |
| PyPI | a **pending** publisher for the project name — PyPI is the one registry that lets you register OIDC before the project exists |
| crates.io | account, and `CARGO_REGISTRY_TOKEN` in `release-crates` |
...
| NuGet | the `NebulaToken` id claimed and `NUGET_API_KEY` issued |
| RubyGems | the gem name claimed and a trusted publisher configured |
| Hex.pm | account and `HEX_API_KEY` |
| pub.dev | the `nebula_token` name claimed and automated publishing enabled from the package admin page |
...
Three of those (npm, RubyGems, pub.dev) can only be configured once a first
version exists, so the honest first release is: publish 1.0.0 by hand from a
clean checkout of the tag, configure the trusted publishers, and let the workflow
own every release after it. That is a one-time exception to "publishes from CI
only", and it is the reason this section exists rather than being folded into the
table above.
...
3. **Prune [`.lycheeignore`](.lycheeignore).** Three of its blocks are temporary
   ...
   | `nebulatoken.dev.*` | the site is deployed | nothing the `links` job looks at — see below |
