...
Dart). NEBULA tokens are opaque---256 bits of CSPRNG output with no claims,
signatures, or structure---so token validity is a property of server-side
...
authorization server must revoke the entire token lineage.
...
providers implement rotation and reuse detection internally with proprietary
and mutually divergent semantics; first-party implementations, which vastly
outnumber them, answer from first principles.
...
\begin{enumerate}[leftmargin=1.4em]
  ...
  selector/verifier split, a six-method storage contract with
  compare-and-set semantics on every state transition, a ten-step
...
it and is planned as an extension.

\paragraph{Split lookup/proof credentials.} The selector/verifier
decomposition used by NEBULA---a public database key alongside a
hashed-at-rest secret---has long been recommended for persistent-login
(``remember-me'') cookies to avoid timing side-channels on credential
lookup~\cite{jaspan2006,owasp-session}. NEBULA adopts it normatively.
...
CSPRNG bytes and constitutes the secret. Tokens exceeding 512 bytes,
...
identifier [N-11]. Raw verifiers and raw device identifiers are never
...
once, and is itself pinned by behavioural scenarios (\texttt{order-01} to
\texttt{order-05}). Steps marked \ddag\ perform a compare-and-set whose
failure is itself a protocol outcome (\S\ref{sec:cas}). A deployment is
...
5 & Status is not \textit{rotated} & revoke family, or grace retry & \texttt{REUSE\_DETECTED} \\
...
consider most consequential since the first draft of this specification.
...
an ordinary retrying client, and it was reproducible against the first
...
\end{tikzpicture}
\caption{Record lifecycle. Edges marked \ddag\ are compare-and-set writes:
they apply only if the record's current status matches the expected one, and
they report whether they applied. A lost compare-and-set yields
...
it. Two behavioural scenarios (\texttt{conflict-01}, \texttt{conflict-02})
drive both losing paths by forcing the compare-and-set to report failure, so
...
approximately 256 bits of entropy and held in an environment variable,
...
Rust, Java, PHP, C\#, Ruby, Elixir, and Dart---each as a single small module
...
\texttt{package:crypto}, in both cases because the language ships no
cryptographic primitives in its standard library. Test-only
dependencies---JSON parsers for the shared vector files, and test
frameworks---are excluded from that count, as they reach no consumer.
...
boundary of the parser, and, per scenario, the returned outcome code, the
returned generation, the family and expiry relations, and the resulting
multiset of record statuses in the store. Agreement is enforced by those
...
off: the constants block [N-4], clock injection [N-3] and the error-code set
[N-38] are asserted by every per-language conformance runner, so they are
executed without being cited by a vector.
