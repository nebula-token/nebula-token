...
No change to this package's behaviour, API, wire format or dependencies. It is
republished so that all ten packages carry one version, as `VERSIONING.md`
section 2 requires.
...
- The conformance runner executes the two new `device_hashing` cases (erratum
  E-1). `Cargo.toml` names an allow-list, so tests reach no consumer.
