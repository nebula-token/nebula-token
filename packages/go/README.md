...
This is a Go **submodule** of the NEBULA monorepo, so its versions are the directory-prefixed tags `packages/go/vX.Y.Z`; `go get …@v1.0.0` resolves against those, and a bare `v1.0.0` tag is not a version of this module ([`VERSIONING.md`](../../VERSIONING.md) §5).
