...
const PACKAGES = {
  typescript: {
    dir: "examples", ext: [".ts"],
    command: '"include": ["src", "examples"]',
    why: "tsconfig.ci-examples.json type-checks src and examples together",
  },
  python: {
    dir: "examples", ext: [".py"],
  ...
  csharp: {
    dir: "examples", ext: [".cs"],
    command: "packages/csharp/examples/*.cs",
...
for (const [pkg, spec] of Object.entries(PACKAGES)) {
  ...
  // Comments do not count. The assertion is that the compiling command itself
  // is still in the workflow, outside a `#` comment.
  const compiles = CI.split("\n").some(
    (line) => !/^\s*#/.test(line) && line.includes(spec.command),
  );
  if (!compiles) {
    problems.push(
