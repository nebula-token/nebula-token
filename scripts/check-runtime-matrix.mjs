...
/**
 * XML comments, removed before anything is read out of a pom or a csproj.
 *
 * Not defensive tidiness: without it a COMMENT can supply the declared floor.
 * `packages/csharp/src/NebulaToken/NebulaToken.csproj` carries a paragraph about
 * target frameworks that mentions the TFM in prose, and this file's regexes take
 * the first match — so a comment naming net10.0 satisfies the check while the
 * real <TargetFramework> says net8.0, and the whole chain stays green while the
 * NuGet package ships against a framework CI never provisions an SDK for.
 *
 * check-licenses.mjs and check-skills.mjs already strip comments before reading
 * these same two files, for the same reason, phrased there as "a check a comment
 * can satisfy is not a check". This file was the outlier.
 */
...
const RUNTIMES = [
  ...
  },
  {
    ...
    floor: (t) => stripXmlComments(t).match(/<maven\.compiler\.release>(\d+)</)?.[1],
  },
  {
  ...
  },
  {
    ...
    declared: (t) => {
      const raw = stripXmlComments(t).match(/<TargetFrameworks?>([^<]+)</)?.[1];
    ...
    alsoCheck(problems, libTfms) {
      ...
      const raw = stripXmlComments(text).match(/<TargetFrameworks?>([^<]+)</)?.[1];
