 ...
 *
 * The previous version enumerated the literal spellings a rule might use, and
 * any glob walked past it. Verified against the real packer: adding
 * `/packages/*\/skills export-ignore` left this gate green while
 * `git archive HEAD | tar t` contained no SKILL.md. `git check-attr` is the same
 * code Composer's `git archive` consults, and — with the ancestor walk below — it
 * covers a rule that matches a DIRECTORY above the file, which is the form that
...
for (const [rel, pkg] of expectedPaths) {
  ...
  //
  // Asserted against the file with HTML COMMENTS REMOVED. Commented-out prose is
  // not something an agent reads, so a skill whose six rules survive only inside
  // <!-- --> has lost them — and that is a normal outcome of an edit that parks a
  // section rather than deleting it. This file already strips comments before
  // reading the pom and the csproj, on the stated grounds that "a check a comment
  // can satisfy is not a check"; the same is true of markdown.
  const stated = stripXmlComments(text);
...
const CHECKS = {
  ...
  php() {
    ...
    if (at === null) return;
    if (exportIgnored(skillPath("php"))) {
      problems.push(`${attrs} export-ignores ${skillPath("php")}, so the PHP dist archive ships without the agent skill (asked of git check-attr, so this covers globs and ancestor directories)`);
    }
    notes.push(`php: git check-attr says neither archive.exclude nor .gitattributes export-ignore drops ${skillPath("php")} from the dist archive`);
  },
