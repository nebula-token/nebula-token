...
/**
 * Does git export-ignore this path? Asked of git, not re-implemented.
 *
 * The previous version of this check enumerated the literal spellings a rule
 * might use — `LICENSE`, `/packages/php/LICENSE` — and any glob walked straight
 * past it. Verified against the real packer: adding
 *
 *   LICEN[SC]E export-ignore
 *   /packages/*\/skills export-ignore
 *
 * left both gates green while `git archive HEAD | tar t` contained neither the
 * licence nor the skill. `git check-attr` is the same code Composer's
 * `git archive` consults, it costs one subprocess, and and with the ancestor walk below it is
 * exact — including for a rule that matches a DIRECTORY above the file, which no
 * amount of pattern-matching on this end would have caught.
 */
function exportIgnored(path) {
  // Ancestors too, and this is the whole point. `git archive` drops the entire
  // subtree of an export-ignored DIRECTORY, but `git check-attr <file>` reports
  // only the attributes matching that exact path — so asking about
  // packages/php/skills/nebula-token-php/SKILL.md says nothing about a rule
  // reading `/packages/*/skills export-ignore`. Mutation-tested: that rule
  // removes the skill from `git archive` output, and the single-path form of this
  // helper reported it fine.
  const parts = path.split("/");
  const candidates = parts.map((_, i) => parts.slice(0, i + 1).join("/"));
  const out = execFileSync("git", ["check-attr", "export-ignore", "--", ...candidates], {
    cwd: ROOT,
    encoding: "utf8",
  });
  // "<path>: export-ignore: set" | "unset" | "unspecified"
  return out
    .split("\n")
    .some((line) => /:\s*export-ignore:\s*set\s*$/.test(line));
}
...
// The BODY of the licence, hashed.
//
// Seven `includes()` checks cannot tell verbatim Apache-2.0 from Apache-2.0 with
// its patent grant negated: replace the text of section 3 with its opposite and
// every marker is still present. Worse, the byte-identity loop below then reports
// 10/10 packages agreeing, so uniformity is mistaken for correctness — which is
// the failure this gate exists to prevent, arrived at from the inside.
//
// The slice from "Apache License" through "END OF TERMS AND CONDITIONS" is the
// normative text and is constant across every copy; the appendix that follows
// carries the copyright line and is not part of it. So the body can be pinned to
// a digest while the appendix stays checked by the assertions below.
const APACHE_2_0_BODY_SHA256 =
...
{
  const start = rootText.indexOf("Apache License");
  const endMarker = "END OF TERMS AND CONDITIONS";
  const end = rootText.indexOf(endMarker);
  if (start < 0 || end < 0) {
    problems.push("root LICENSE has no Apache-2.0 body to hash — the markers above should already have caught this");
  } else {
    const body = rootText.slice(start, end + endMarker.length);
    const got = createHash("sha256").update(body, "utf8").digest("hex");
    if (got !== APACHE_2_0_BODY_SHA256) {
      problems.push(
        `root LICENSE body is not the verbatim Apache-2.0 text: sha256 ${got}, expected ${APACHE_2_0_BODY_SHA256}. ` +
          "One word of the grant is one word too many — diff it against a canonical copy before changing this constant.",
      );
    }
  }
}
...
const CHECKS = {
  ...
  php() {
    ...
    // Exactly one composer.json, at the root. Three files call this invariant
    // load-bearing — this gate, ci.yml and AGENTS.md — and nothing enforced it: a
    // second packages/php/composer.json with its own licence and a `version` key
    // passed all eleven gates, because every reader here names the root file
    // explicitly. PHP tooling inside packages/php would resolve the new one.
    const manifests = execFileSync("git", ["ls-files", "--", "*composer.json"], {
      cwd: ROOT,
      encoding: "utf8",
    })
      .split("\n")
      .filter(Boolean);
    if (manifests.length !== 1 || manifests[0] !== "composer.json") {
      problems.push(
        `there must be exactly one composer.json, at the repository root, because Packagist reads a manifest from a root only — found ${manifests.length}: ${manifests.join(", ")}`,
      );
    }
    ...
    if (!existsSync(join(ROOT, attrs))) {
      ...
      for (const path of ["LICENSE", "packages/php/LICENSE"]) {
        if (exportIgnored(path)) {
          problems.push(`${attrs} export-ignores ${path}, so the PHP dist archive ships without the licence text (asked of git check-attr, so this covers globs and ancestor directories)`);
        }
      }
    }
  },
