...
// by the runtime's floor. NOTHING currently uses that substitution: the .NET row
// carried it while its commands had a `-f ${{ matrix.framework }}`, and that flag
// is gone (it split a solution build in two and raced their deps-file writes).
// Ruby is the only remaining {anchor, show} entry and contains no matrix
// expression. `substitute()` is kept for the next shown command that needs one.
//
...
const PACKAGES = [
  ...
  // One TFM, so CI's commands are runnable as they stand: nothing to substitute,
  // and no `-f`. On a SOLUTION build `-f` is a global property that splits the
  // library into two project instances and races their GenerateDepsFile writes
  // (MSB4018). It went with the TargetFrameworks overrides above.
