...
function check() {
  ...
  for (const s of SERVICES) {
    ...
    // Exactly one is in that position — the .NET one, because a single official
    // image carries a single SDK and every command needs a flag CI does not use.
    // That has to be declared, not discovered by a reader.
