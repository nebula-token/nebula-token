...
| .NET | `packages/csharp` | net10.0 | `dotnet restore && dotnet build -c Release -f net10.0 --no-restore && dotnet test -c Release -f net10.0 --no-build` |
...
**.NET targets one framework, `net10.0`, and needs only SDK 10.** It
multi-targeted `net8.0;net10.0` until .NET 8's LTS window closed, and that cost
a dual-SDK requirement everywhere: an older SDK cannot even *restore* a project
naming a newer framework, so every command carried a `-p:TargetFrameworks=`
override. Those are gone. If a second framework is ever added back, it returns
to `docker/compose.yml`, `scripts/check-agents-md.mjs` and the CI setup step
together — and **`NETSDK1045` advises "target .NET 8.0 or lower"; do not.**
Editing `TargetFramework` down drops the only supported runtime.
